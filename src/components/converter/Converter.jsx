...
const Converter = () => {
    ...
    // Зачем при флипе он берёт два запроса на сервер, хотя до этого управлялся одним - непонятно.
    // Тут вообще можно было положить конвертацию в кеш и не вызывать его при каждом чихе пользователя, но задачи точной не стояло, вдруг пользователю надо,
    // чтоб были прям самые актуальные данные, так что это решение тоже имеет право на ошибку
    // Но флип всё равно закосяченный

    return (
        <div className='converter'>
            {/* Пока статус loading, показывает всё те же цифры, хотя валюты он уже поменял, что при медленном ответе от сервера может сильно смутить пользователя */}
